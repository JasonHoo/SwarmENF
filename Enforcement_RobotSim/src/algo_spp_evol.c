...
void InitializeFlockingParams (flocking_model_params_t * FlockingParams) {
    ...
    //added by HC
    FlockingParams->innerDistance = 5000;
    FlockingParams->SwarmLocation[0] = ArenaCenterX;
    FlockingParams->SwarmLocation[1] = ArenaCenterY;
    FlockingParams->SwarmLocation[2] = 0;
    FlockingParams->maxX = 0;
    FlockingParams->minX = 0;
    FlockingParams->maxY = 0;
    FlockingParams->minY = 0;
    FlockingParams->WatchTimer = 0;
    FlockingParams->NumberofActiveAgents = 100;
}
...
void CalculatePreferredVelocity(double *OutputVelocity,
    ...
    /* added by HC*/
    static double innerDist = 0.0;
    static double SwarmLocation[2];
    NullVect(SwarmLocation, 2);
...
//     Instrument of the Swarm_innerDistance calculation ---------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        innerDist = Swarm_innerDistance(DistanceFromNeighbour, FlockingParams);
      // printf("The safety distance in the swarm is  %f \n", innerDist );

        Swarm_location(AgentsCoordinates, Phase, FlockingParams);
        // printf("Now the location of this swarm is  %f , %f \n", FlockingParams->SwarmLocation[0],FlockingParams->SwarmLocation[1]);

//     Instrument of the Swarm_innerDistance calculation ---------------------------------------------------------------------------------------------------------------------------------------------------------------

        /* Check if we interact at all */
        if (DistanceFromNeighbour >= 2*0.5*R_0) {
            continue;
        }
        n += 1;
    }
...
//    	UnitVect(NormalizedAgentsVelocity, NormalizedAgentsVelocity);
//    	MultiplicateWithScalar(NormalizedAgentsVelocity, NormalizedAgentsVelocity,
//    	            V_Flock-0.0002*n*n, (int) Dim);
//    	if(V_Flock-0.0002*n*n > 0.1){
//    	V_Flock = V_Flock-0.0002*n*n;
//    	}
    }
    else{
        V_Flock = temp;
...
//added by HC --  Instrument for the RE monitor

double Swarm_innerDistance(double DistanceFromNeighbour_in, flocking_model_params_t * FlockingParams){
        if(DistanceFromNeighbour_in < FlockingParams->innerDistance ){
        	FlockingParams->innerDistance = DistanceFromNeighbour_in;
        }
        return FlockingParams->innerDistance;
}

void Swarm_location(double *AgentLocation_in,phase_t * Phase, flocking_model_params_t * FlockingParams){
//	    if(AgentLocation_in[0]<FlockingParams->minX){
//                FlockingParams->minX = AgentLocation_in[0];
//	    }
//	    else if(AgentLocation_in[0]>FlockingParams->maxX){
//	    	    FlockingParams->maxX = AgentLocation_in[0];
//	    }
//	    else if(AgentLocation_in[1]<FlockingParams->minY){
//	    	    FlockingParams->minY = AgentLocation_in[1];
//	    }
//	    else if(AgentLocation_in[1]>FlockingParams->maxY){
//	            FlockingParams->maxY = AgentLocation_in[1];
//	    }
        ...
        int i;
	    for( i = 0; i<100;  i++){
	    	double *tmpCoordinates = Phase->Coordinates[i];
	    	   if(tmpCoordinates[0]<FlockingParams->minX){
	    	                FlockingParams->minX = tmpCoordinates[0];
	    		    }
	    		    else if(tmpCoordinates[0]>FlockingParams->maxX){
	    		    	    FlockingParams->maxX = tmpCoordinates[0];
	    		    }
	    		    else if(tmpCoordinates[1]<FlockingParams->minY){
	    		    	    FlockingParams->minY = tmpCoordinates[1];
	    		    }
	    		    else if(tmpCoordinates[1]>FlockingParams->maxY){
	    		            FlockingParams->maxY = tmpCoordinates[1];
	    		    }
	    }
	    double swarmCenterX = (FlockingParams->minX + FlockingParams->maxX) / 2;
	    double swarmCenterY = (FlockingParams->minY + FlockingParams->maxY) / 2;
	    FillVect(FlockingParams->SwarmLocation, swarmCenterX, swarmCenterY, 0);
}
