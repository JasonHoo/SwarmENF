...
//--------------HC------------------------------------------------------------------------
void HandleSpecialDispEvent(int flag, int state,
                             flocking_model_params_t * FlockingParams,
                             vizmode_params_t * VizParams) {

    if (VizParams->TwoDimViz) {
        CHANGE_PARAMETER(ArenaCenterX,  VizParams->CenterX -200000 + flag);
        CHANGE_PARAMETER(ArenaCenterY,  VizParams->CenterY - flag/5 + state*6000);
    }

    SetupVertices(VizParams);
    CreateObstacleVertexSet(&obstacles, VizParams);
}
void HandleSpecialEnfEvent(int Enf_x, int Enf_y,
                            flocking_model_params_t * FlockingParams,
                            vizmode_params_t * VizParams) {

    if (VizParams->TwoDimViz) {
        CHANGE_PARAMETER(ArenaCenterX,  VizParams->CenterX + Enf_x);
        CHANGE_PARAMETER(ArenaCenterY,  VizParams->CenterY + Enf_y);
    }
