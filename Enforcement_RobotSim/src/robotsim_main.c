...
bool *AgentsInDanger;
int Collisions;
clock_t start,end;  //--HC-----------------
int itor = 1;  //--HC-----------------
int enforceTrigger = 0; //--HC-----------------
...
void Initialize() {
    ...
    ActualVizParams.LengthOfTail = 1000;
...
//---HC-----------------------------------------------------------------------------------------------------------------------------------------------------------
                    ...
                    DrawLine(-1,      0.68,      1,       0.68   ,        	0.01,   ActualColorConfig.VelocityArrowColor );
                    DrawLine(-1,      -0.68,      1,     -0.68   ,        	0.01,   ActualColorConfig.VelocityArrowColor );
                    DrawLine(-0.52,   0.68,      -0.52,    -0.68   ,        	0.004,  col );
                    DrawLine(0.52,    0.68,      0.52,      -0.68   ,        	0.004,   col );
                    DrawLine(0.52,   0,      -0.52,      0   ,        	0.004,   col );
                    static char RegionLabelA[10];
                    static char RegionLabelB[10];
                    static char RegionLabelB2[10];
                    static char RegionLabelC[10];
                    sprintf(RegionLabelA, "Region A");
                    sprintf(RegionLabelB, "Region B");
                    sprintf(RegionLabelB2, "Region B' ");
                    sprintf(RegionLabelC, "Region C");
                
                    DrawString(-0.85, 0.25, GLUT_BITMAP_TIMES_ROMAN_10, RegionLabelA,
                                      ActualColorConfig.VelocityArrowColor);
                    DrawString(-0.25, 0.45, GLUT_BITMAP_TIMES_ROMAN_10, RegionLabelB,
                                      ActualColorConfig.VelocityArrowColor);
                    DrawString(-0.25, -0.25, GLUT_BITMAP_TIMES_ROMAN_10, RegionLabelB2,
                                      ActualColorConfig.VelocityArrowColor);
                    DrawString(0.85, 0.25, GLUT_BITMAP_TIMES_ROMAN_10, RegionLabelC,
                                      ActualColorConfig.VelocityArrowColor);
//starup a timeinterval enforcement-----------------------------------------------------------------------------------------
                    int tempstart = start;
                    int state = 0;
                    start = clock();
                    if(start >= tempstart){
                    ...
                    if( (diff>3000000) && (ActualUnitParams.Wind_Magn_Avg.Value >= 200) ){
                        itor = itor + 1;
                       // printf("itor now is : %d \n", itor);
                        if(itor>=44){
                            state = itor-43;
                        }
                        printf("SwarmLocation_x now is : %f ,  ", ActualFlockingParams.SwarmLocation[0]);
                        printf("SwarmLocation_y now is : %f ,  ", ActualFlockingParams.SwarmLocation[1]);
                        printf("SwarmLocation_Z now is : %f  \n", ActualFlockingParams.SwarmLocation[2]);
                        if(ActualFlockingParams.SwarmLocation[0] >= 78000 && ActualFlockingParams.SwarmLocation[1] <= 0 ){
                            enforceTrigger = 1;
                            itor = 0;
                        }
                        ...
                        if(enforceTrigger == 1){
                            itor = itor + 1;
                            HandleSpecialEnfEvent(20000+6000*itor, 40000, &ActualFlockingParams, &ActualVizParams);
                        }
                        else{
                            HandleSpecialDispEvent(200*diff/100000*itor, state, &ActualFlockingParams, &ActualVizParams);
                        }
                        start = end;
                    }
