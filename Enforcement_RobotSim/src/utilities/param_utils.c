...
void ChangeUnitModelParameter(unit_model_params_t * params, int WhichParam,
    ...
    case 14:
    {
        params->Robot1_Coord_X.Value += StepSize * 1.0;
        if (params->Robot1_Coord_X.Value < 0.0) {
            params->Robot1_Coord_X.Value = 0.0;
        }
    }
        break;
    }
...
void SetNamesOfUnitModelParams(unit_model_params_t * UnitParams) {
    ...
    sprintf(UnitParams->Robot1_Coord_X.UnitOfMeas, "m");
    ...
    sprintf(UnitParams->Robot1_Coord_X.Name, "Robot1_Coord_X");
...
void GetUnitModelParamsFromFile(unit_model_params_t * UnitParams,
    ...
    if (NULL == InputFile) {
        ...
        UnitParams->Robot1_Coord_X.Value = 0.0;
    ...
    while (fgets(line, sizeof(line), InputFile) != NULL) {
        ...
        if (*start == ';' || *start == '#') {
            ...
            if (strcmp(ReadedName, "tau_PID_XY") == 0) {
                ...
                NumberOfReadedNames++;
            }else if (strcmp(ReadedName, "Robot1_Coord_X") == 0) {
                UnitParams->Robot1_Coord_X.Value = atof(ReadedValue);
                NumberOfReadedNames++;
            }
        }
    }
    ...
    if (NumberOfReadedNames < 16) {

        printf("16 paramers are necessary in \n'%s' \n\nRequired format (example):\n\n", recover_fileName(InputFile));
        printf("tau_PID_XY=1\n");
        printf("tau_PID_Z=1\n");
        printf("a_max=600\n");
        printf("R_C=10000\n");
...
void SaveUnitModelParamsToFile(FILE * OutputFile_Unit,
    ...
    fprintf(OutputFile_Unit, "Robot1_Coord_X=%lf\n",
                UnitParams->Robot1_Coord_X.Value);
